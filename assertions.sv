...
//zero 
property pr1;
	@(posedge clk) status[0] && status[1];
endproperty
property1: assert property (pr1) $display("zero-inf"); else;

property pr2;
	@(posedge clk) status[0] && status[2];
endproperty
property2: assert property (pr2) $display("zero-nan"); else;

property pr4;
	@(posedge clk) status[0] && status[4];
endproperty
property4: assert property (pr4) $display("zero-huge"); else;

property pr3;
	@(posedge clk) status[1] && status[2]
endproperty
property3: assert property (pr3) $display("inf-nan"); else;

property pr5;
	@(posedge clk) status[1] && status[3];
endproperty
property5: assert property (pr5) $display("inf-tiny"); else;

property pr6;
	@(posedge clk) status[2] && status[3];
endproperty
property6: assert property (pr6) $display("nan-tiny"); else;

property pr7;
	@(posedge clk) status[2] && status[4];
endproperty
property7: assert property (pr7) $display("nan-huge"); else;

property pr8;
	@(posedge clk) status[2] && status[5];
endproperty
property8: assert property (pr8) $display("nan-inexact"); else;

property pr9;
	@(posedge clk) status[6];
endproperty
property9: assert property (pr9) $display("unused"); else;

property pr10;
	@(posedge clk) status[7];
endproperty
property10: assert property (pr10) $display("div by 0"); else;
property pr11;
	@(posedge clk) status[3] && status[4];
endproperty
...
property pr2;
	@(posedge clk) status[1] && (z[30:23] == {8{1'b1}});
endproperty
property2: assert property (pr2) $display("inf asserted"); else;

property pr3;
	@(posedge clk) ((a[30:23] == 8'b0 && b[30:23] == {8{1'b1}}) || (b[30:23] == 8'b0 && a[30:23] == {8{1'b1}})) ##2 status[2] ;
endproperty
property3: assert property (pr3) $display("nan asserted"); else;

property pr4;
	@(posedge clk) status[4] && ( z[30:23] == {8{1'b1}} || z[30:0] == {{7{1'b1}},1'b0,{23{1'b1}}} );
endproperty
property4: assert property (pr4) $display("huge asserted"); else;

property pr5;
	@(posedge clk) status[3] && ( z[30:23] == 8'b0 || z[30:0] == {8'b1,23'b0} );
endproperty
