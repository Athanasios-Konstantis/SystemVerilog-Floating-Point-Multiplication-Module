`include "globals.sv"
import global_types::*;
module main_module(input wire[31:0] a, input wire[31:0] b, input logic[2:0] rnd, output logic[0:7] status, output logic[31:0] z);
	rounding_mode rndm;
	...
	normalization norm(.exponent(added_exp),.P(mult_mantissa),.normalized_exponent(normalized_exponent),.normalized_mantissa(normalized_mantissa),.guard_bit(guard_bit),.sticky_bit(sticky_bit));
	rounding round(normalized_mantissa,guard_bit,sticky_bit,sign_xor,rndm,normalized_exponent,post_round_mantissa,post_round_exponent, inexact);
...
always @* begin
	...
	status = {zero_f,inf_f,nan_f,tiny_f,huge_f,inexact_f,1'b0,1'b0};
end
endmodule
